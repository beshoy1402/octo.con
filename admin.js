...
async function devSaveReceiptSettings() {
  const fields = [
    'receipt_header','receipt_footer','receipt_font','receipt_width',
    'receipt_accent_hex','receipt_show_wifi','receipt_show_images',
    'receipt_divider','receipt_css'
  ];
  const inputMap = {
  ...
  try {
    await Promise.all(Object.entries(inputMap).map(([key, id]) => {
      const el = document.getElementById(id);
      return el ? DB.updateSetting(key, el.value) : null;
    }));
    ...
    Object.assign(Admin.settings, Object.fromEntries(
      Object.entries(inputMap).map(([key, id]) => {
        const el = document.getElementById(id);
        return [key, el ? el.value : ''];
      })
    ));
