...
function renderAnalytics() {
  ...
  setTimeout(()=>{
    ...
    if(ctx&&typeof Chart!=='undefined'){if(Admin.charts.revenue)Admin.charts.revenue.destroy();Admin.charts.revenue=new Chart(ctx,{type:'line',data:{labels:Object.keys(dayMap),datasets:[{label:'Revenue',data:Object.values(dayMap),borderColor:'#7C3AED',backgroundColor:'rgba(124,58,237,.08)',borderWidth:2,fill:true,tension:.4,pointRadius:4,pointBackgroundColor:'#7C3AED'}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{x:{ticks:{color:'#666',font:{size:11}},grid:{color:'rgba(255,255,255,.04)'}},y:{ticks:{color:'#666',font:{size:11},callback:v=>`ج.م ${v}`},grid:{color:'rgba(255,255,255,.04)'}}}}})}
      ...
      <div class="settings-card" style="grid-column:1/-1">
        ...
        <div style="margin-bottom:24px">
          ...
          <div style="display:flex;align-items:center;gap:16px;flex-wrap:wrap">
            <input type="color" value="${s.accent_color||'#7C3AED'}" id="accentColorPicker" oninput="updateAccentLive(this.value)" style="width:52px;height:40px;border:none;cursor:pointer;border-radius:8px;background:transparent">
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              ${['#7C3AED','#e11d48','#7c3aed','#2563eb','#0891b2','#059669','#d97706','#dc2626'].map(c=>`
                <button onclick="updateAccentLive('${c}');document.getElementById('accentColorPicker').value='${c}'"
                  style="width:32px;height:32px;border-radius:50%;background:${c};border:3px solid ${(s.accent_color||'#7C3AED')===c?'#fff':'transparent'};cursor:pointer;transition:border-color .2s" title="${c}"></button>`).join('')}
            </div>
